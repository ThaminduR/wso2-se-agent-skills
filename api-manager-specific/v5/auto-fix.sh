...
usage() {
  ...
  echo "  2. plan           Produce a fix plan from the issue + comments + referenced issues/PRs"
  echo "  3. fix            Implement, build, patch, and dev-test the fix per the plan"
  echo "  4. verify-fix     Build, patch the product, and verify the fix resolves the issue"
  echo "  5. create-tests   Write unit and integration tests for the fix"
  echo "  6. submit-fix     Create PRs for the fix and generate a fix report"
  echo ""
  echo "Options:"
  echo "  --steps <steps>   Comma-separated steps to run (default: all)"
  echo "                    Accepts names or numbers: reproduce,plan,fix or 1,2,3"
  ...
  echo ""
  echo "  # Skip reproduction — just plan and implement"
  echo "  ./auto-fix.sh --steps plan,fix https://github.com/wso2/product-apim/issues/12345"
  echo ""
  ...
  echo "  ./auto-fix.sh --steps reproduce,plan https://github.com/wso2/product-apim/issues/12345"
  echo ""
...
normalize_step() {
  case "$1" in
    ...
    2|plan)          echo "plan" ;;
    3|fix)           echo "fix" ;;
    4|verify-fix)    echo "verify-fix" ;;
    5|create-tests)  echo "create-tests" ;;
    6|submit-fix)    echo "submit-fix" ;;
    *) echo "Error: Unknown step '$1'. Valid: reproduce, plan, fix, verify-fix, create-tests, submit-fix (or 1-6)"; exit 1 ;;
  esac
}
...
if should_run "plan"; then
  STEP2_LOG="$LOG_DIR/issue-${ISSUE_NUMBER}-2-plan-${TIMESTAMP}.log"
  echo "=== [2/6] Planning fix... ==="
  STEP_START=$(date +%s)
  clean_environment
  echo "  Log: $STEP2_LOG"
  run_claude_streaming "$STEP2_LOG" "/plan $ISSUE_URL"
  STEP_END=$(date +%s)
  echo "  Duration: $(( STEP_END - STEP_START ))s"
  if [ ! -f ".ai/plan-${ISSUE_NUMBER}.md" ]; then
    echo "Error: Planning failed — no plan artifact found at .ai/plan-${ISSUE_NUMBER}.md"
    exit 1
  fi
  echo ""
  echo "=== Plan produced ==="
...
if should_run "fix"; then
  STEP3_LOG="$LOG_DIR/issue-${ISSUE_NUMBER}-3-fix-${TIMESTAMP}.log"
  echo "=== [3/6] Implementing fix... ==="
  STEP_START=$(date +%s)
  clean_environment
  echo "  Log: $STEP3_LOG"
  run_claude_streaming "$STEP3_LOG" "/fix $ISSUE_NUMBER"
  STEP_END=$(date +%s)
  echo "  Duration: $(( STEP_END - STEP_START ))s"
  echo ""
...
if should_run "verify-fix"; then
  STEP4_LOG="$LOG_DIR/issue-${ISSUE_NUMBER}-4-verify-fix-${TIMESTAMP}.log"
  echo "=== [4/6] Verifying fix... ==="
  STEP_START=$(date +%s)
  clean_environment
  echo "  Log: $STEP4_LOG"
  run_claude_streaming "$STEP4_LOG" "/verify-fix $ISSUE_URL"
  STEP_END=$(date +%s)
  echo "  Duration: $(( STEP_END - STEP_START ))s"
  echo ""
...
if should_run "create-tests"; then
  STEP5_LOG="$LOG_DIR/issue-${ISSUE_NUMBER}-5-create-tests-${TIMESTAMP}.log"
  echo "=== [5/6] Creating tests... ==="
  STEP_START=$(date +%s)
  clean_environment
  echo "  Log: $STEP5_LOG"
  run_claude_streaming "$STEP5_LOG" "/create-tests $ISSUE_NUMBER"
  STEP_END=$(date +%s)
  echo "  Duration: $(( STEP_END - STEP_START ))s"
  echo ""
  echo "=== Tests created ==="
...
if should_run "submit-fix"; then
  STEP6_LOG="$LOG_DIR/issue-${ISSUE_NUMBER}-6-submit-fix-${TIMESTAMP}.log"
  echo "=== [6/6] Submitting PRs... ==="
  STEP_START=$(date +%s)
  clean_environment
  echo "  Log: $STEP6_LOG"
  run_claude_streaming "$STEP6_LOG" "/submit-fix $ISSUE_NUMBER"
  STEP_END=$(date +%s)
  echo "  Duration: $(( STEP_END - STEP_START ))s"
  echo ""
  echo "=== PRs submitted ==="
